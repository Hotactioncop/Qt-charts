...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->LoadButton, &QPushButton::clicked, [=] {
    ...
    QString newSet = curName + "_" + ui->comboBox->currentText() + "_" +
                     ui->dateFrom->date().toString("dd.MM.yyyy") + "_" +
                     ui->dateTo->date().toString("dd.MM.yyyy");
    getData(curName);
    this->setWindowTitle(curName + " " + ui->comboBox->currentText() + " " +
                         ui->dateFrom->date().toString("dd.MM.yyyy") + "-" +
                         ui->dateTo->date().toString("dd.MM.yyyy"));

    curSets = newSet;
    ui->widgetManage->setEnabled(true);
  });
...
void MainWindow::drawSceneItems(int put) {
  ...
  if (ui->pushButtonVolume->isChecked()) {
    scene->addLine(
        2, sceneheight - paddingBottom - paddingVolume + 2, sceneWidth,
        sceneheight - paddingBottom - paddingVolume + 2,
        pen); //Наносим черту разделяющую основной график от графика объема
  }
  ...
  double screenSize =
      (sceneheight - paddingTop - paddingBottom - paddingVolume) /
      (maxSet -
  ...
  for (int i = barPos; i < (barPos + (sceneWidth) / (barWidth + barSpace));
    ...
    if (barList[i].close >= barList[i].open) { //Если в рост
      if (i && barList[i - 1].time.date().month() !=
                   barList[i].time.date().month()) { //Если начало дня, то
                                                     //добавляем вертикальный
                                                     //разделитель(линию)
        scene->addLine(put + barWidth / 2, 2, put + barWidth / 2,
                       sceneheight - paddingBottom - paddingVolume, penLine);
      }
      CandleBar *bbbar = new CandleBar(
          ...
          put + barWidth / 2,
          bonusSpace + (maxSet - barList[i].low) * screenSize, 1, type, scene);
      scene->addItem(bbbar);
      ...
      bbbar = nullptr;
      if (showATR) {
        bezierList.push_back(
            QPointF(put + barWidth / 2,
                    int(bonusSpace + (maxSet - atrList[i]) * screenSize)));
      }
      ...
      CandleBar *bbbar = new CandleBar(
          ...
          bonusSpace + (maxSet - barList[i].high) * screenSize,
          put + barWidth / 2,
          bonusSpace + (maxSet - barList[i].low) * screenSize, 0, type, scene);
      scene->addItem(bbbar);
      candleVector.push_back(bbbar);
      connect(bbbar, SIGNAL(sendIDinfo(int &)), this, SLOT(showBarInfo(int &)));
      connect(bbbar, &CandleBar::clearIDinfo,
  ...
  pixelsize = (maxSet - minSet) / double(sceneheight - paddingTop -
                                         paddingBottom - paddingVolume + 2);
  double begin = minSet - pixelsize * (paddingTop - paddingBottom + 3) +
                 pixelsize * 32; //Начинаю заполнять не с самого низа
    ...
    if (whenToPutDate) {
      ...
      if (barPos + whichBar < 0) {
        if (format_type == day) {
          ...
          QGraphicsTextItem *textitem = scene->addText(
              barList[0].time.addDays(-1).toString("dd MMM hh:mm"), font);
          textitem->setDefaultTextColor(Qt::white);
          textitem->moveBy(i, sceneheight - 27);
        }
        whenToPutDate = false;
        ...
        if (format_type == day) {
          ...
          QGraphicsTextItem *textitem = scene->addText(
              barList[barPos + whichBar].time.toString("dd MMM hh:mm"), font);
          textitem->setDefaultTextColor(Qt::white);
          textitem->moveBy(i, sceneheight - 27);
        }
...
void MainWindow::drawSceneLine(int put) {
  ...
  double screenSize =
      (sceneheight - paddingTop - paddingBottom - paddingVolume) /
      (*iSet.rbegin() -
       *iSet.begin()); //-70, чтобы оставить 30 пикселей снизу и сверху от бара
  int i = barPos;
  if (ui->pushButtonVolume_2->isChecked()) {
    drawVolume2(3);
  }
  ...
  for (; i < (barPos + (sceneWidth) / (barWidth + barSpace));
       i++) { //Пробегаем от индекса бара, содержащегося в контейнере barList
    if (i &&
        barList[i - 1].time.date().month() !=
            barList[i].time.date().month()) { //Если начало дня, то добавляем
  ...
  scene->addRect(2, 2, sceneWidth, sceneheight - paddingBottom,
                 pen); //Каждый раз рисуем рамку экрана
  if (ui->pushButtonVolume->isChecked()) {
    scene->addLine(2, sceneheight - paddingBottom - paddingVolume + 2,
                   sceneWidth, sceneheight - paddingBottom - paddingVolume + 2,
                   pen); //Каждый раз рисуем рамку экрана
  }
  put = 3;
  drawVolume(3);
  pixelsize = (maxSet - minSet) / double(sceneheight - paddingTop -
                                         paddingBottom - paddingVolume + 2);
  double begin =
      minSet - pixelsize * (paddingTop - paddingBottom + 3) + pixelsize * 32;
  for (int i = sceneheight - 28 - paddingVolume - 32; i > 10; i -= 32) {
    scene->addLine(sceneWidth + 2, i, sceneWidth + 5, i, pen);
    QGraphicsTextItem *textitem =
        scene->addText(QString::number(begin, 'f', 5), font);
...
void MainWindow::drawVolume(int) {
  ...
  QLinearGradient gradient = QLinearGradient(
      2, sceneheight - paddingVolume - paddingBottom, 2, sceneheight);
  ...
  for (int i = barPos; i < (barPos + (sceneWidth) / (barWidth + barSpace));
       i++) { //Пробегаем от индекса бара, содержащегося в контейнере barList
    scene->addRect(
        QRectF(QPointF(put, sceneheight - paddingBottom - paddingVolume + 10 +
                                (maxVolume - barList[i].volume) * pixelsize),
               QPointF(put + barWidth, sceneheight - paddingBottom)),
        pen, brush);
    put += (barWidth + barSpace);
  }
  ...
  for (int i = sceneheight - 28;
       i > sceneheight - paddingBottom - paddingVolume; i -= 32) {
    scene->addLine(sceneWidth + 2, i, sceneWidth + 5, i, pen);
    QGraphicsTextItem *textitem =
        scene->addText(QString::number(begin / 500 * 500), font);
    textitem->setDefaultTextColor(Qt::white);
    textitem->moveBy(sceneWidth + 6, i - 8);
    begin += pixelsize * 32;
    textitem = nullptr;
...
void MainWindow::drawVolume2(int) {
  ...
  for (int i = barPos; i < (barPos + (sceneWidth) / (barWidth + barSpace));
       i++) { //Пробегаем от индекса бара, содержащегося в контейнере barList
    scene->addRect(
        QRectF(QPointF(put, paddingBottom +
                                (maxVolume - barList[i].volume) * pixelsize),
               QPointF(put + barWidth, sceneheight - paddingBottom)),
        pen, brush);
    put += (barWidth + barSpace);
...
void MainWindow::showBarInfo(int &ID) {
  if (format_type == day) {
    textInfo = scene->addText(
        QString("High: %1\tOpen: %3\tDate: %5\nLow: %2\tClose: %4")
            .arg(barList[ID].high)
            .arg(barList[ID].low)
            .arg(barList[ID].open)
    ...
    textInfo = scene->addText(
        QString("High: %1\tOpen: %3\tDate: %5\nLow: %2\tClose: %4")
            ...
            .arg(barList[ID].open)
            .arg(barList[ID].close)
            .arg(barList[ID].time.toString("yyyy MMM dd hh:mm")),
        fontInfo);
  }
...
void MainWindow::fillStatusBar(QPointF &point) {
  ...
  if (point.rx() > 2 && point.rx() < sceneWidth && point.ry() > 2 &&
    ...
    if (ui->pushButtonCursor->isChecked()) {
      ...
      if (rectDown == nullptr) {
        pixelsize =
            (maxSet - minSet) / double(sceneheight - paddingTop -
                                       paddingBottom - paddingVolume + 2);
        ...
        if (format_type == day) {
          ...
          rectDown = scene->addRect((point.rx() - 25 > 3) ? point.rx() - 25 : 3,
                                    sceneheight - 27, 60, 12, boxinfo,
                                    QBrush(Qt::white));
          textDown =
              scene->addText(barList[bar].time.toString("dd MMM hh:mm"), font);
        }
        rectRight = scene->addRect(sceneWidth + 3,
                                   (point.ry() - 6 > 3) ? point.ry() - 6 : 3,
                                   60, 12, boxinfo, QBrush(Qt::white));
      ...
      if (drawLineInCrosses && crossLine3 == nullptr) {
        ...
        rectRight2 = scene->addRect(
            sceneWidth + 3,
            (prevMousePoint.ry() - 6 > 3) ? prevMousePoint.ry() - 6 : 3, 60, 12,
        ...
        if (format_type == day) {
          ...
          rectDown2 = scene->addRect(
              (prevMousePoint.rx() - 25 > 3) ? prevMousePoint.rx() - 25 : 3,
              sceneheight - 27, 60, 12, boxinfo, QBrush(Qt::white));
          textDown2 =
              scene->addText(barList[bar].time.toString("dd MMM hh:mm"), font);
        }
...
void MainWindow::getData(QString data) {
  {
    ...
    for (int curYear = yearBegin; curYear <= yearEnd; curYear++) {
      Comp.setFileName(QString(":/Docs/%1_%2_01.01.%3_31.12.%4.txt")
                           .arg(data)
                           .arg(ui->comboBox->currentText())
                           .arg(QString::number(curYear))
                           .arg(QString::number(curYear)));
      if (Comp.open(QIODevice::ReadOnly | QIODevice::Text)) {
        while (!Comp.atEnd()) {
          ...
          if (QDate::fromString(strLine.at(2), "dd/MM/yy").addYears(100) <
              ui->dateFrom->date()) {
            continue;
          }
          if (QDate::fromString(strLine.at(2), "dd/MM/yy").addYears(100) >
          ...
          if (xTime.date().dayOfWeek() == 6 || xTime.date().dayOfWeek() == 7) {
            continue;
          }
          Bar *barX = new Bar(xTime, QString(strLine.at(4)).toDouble(),
                              QString(strLine.at(5)).toDouble(),
                              QString(strLine.at(6)).toDouble(),
                              QString(strLine.at(7)).toDouble(),
                              QString(strLine.at(8)).toInt());
...
void MainWindow::addLastPrice(double newClose) {
  ...
  if (format_type == day) {
    if (QDate::currentDate() == barList[barList.size() - 1].time.date()) {
      ...
      Bar *barX = new Bar(QDateTime::currentDateTime(), newClose, newClose,
                          newClose, newClose, 0);
      barList.push_back(*barX);
      delete barX;
    }
  } else if (format_type == hour) {
    if (QDateTime::currentDateTime() >= barList[barList.size() - 1].time &&
        QDateTime::currentDateTime() <=
            barList[barList.size() - 1].time.addSecs(3600)) {
      barList[barList.size() - 1].close = newClose;
    } else {
      Bar *barX = new Bar(QDateTime::currentDateTime(), newClose, newClose,
                          newClose, newClose, 0);
    ...
    if (QDateTime::currentDateTime() >= barList[barList.size() - 1].time &&
        QDateTime::currentDateTime() <=
            barList[barList.size() - 1].time.addSecs(1800)) {
      barList[barList.size() - 1].close = newClose;
    } else {
      Bar *barX = new Bar(QDateTime::currentDateTime(), newClose, newClose,
                          newClose, newClose, 0);
    ...
    if (QDateTime::currentDateTime() >= barList[barList.size() - 1].time &&
        QDateTime::currentDateTime() <=
            barList[barList.size() - 1].time.addSecs(900)) {
      barList[barList.size() - 1].close = newClose;
    } else {
      Bar *barX = new Bar(QDateTime::currentDateTime(), newClose, newClose,
                          newClose, newClose, 0);
    ...
    if (QDateTime::currentDateTime() >= barList[barList.size() - 1].time &&
        QDateTime::currentDateTime() <=
            barList[barList.size() - 1].time.addSecs(60)) {
      barList[barList.size() - 1].close = newClose;
    } else {
      Bar *barX = new Bar(QDateTime::currentDateTime(), newClose, newClose,
                          newClose, newClose, 0);
...
void MainWindow::drawRedRect() {
  ...
  //текущей цены------------------------------------------------- Рисуем красный
  //квадрат справа с текущей ценой
  ...
  if (curPrice >= minSet && curPrice <= maxSet) {
    rectRightPrice = scene->addRect(sceneWidth + 7, sceneheight - 28 - 5 - diff,
                                    50, 12, boxinfo, QBrush(Qt::red));
    scene->addLine(sceneWidth + 2, sceneheight - 27 - diff, sceneWidth + 6,
                   sceneheight - 27 - diff, QPen(QColor(Qt::white)));
    ...
    rectRightPrice = scene->addRect(sceneWidth + 7, sceneheight - 33, 50, 12,
                                    boxinfo, QBrush(Qt::red));
    scene->addLine(sceneWidth + 2, sceneheight - 28, sceneWidth + 6,
                   sceneheight - 28, QPen(QColor(Qt::white)));
    rectRightPrice->setBrush(Qt::red);
    textRightPrice = scene->addText(QString::number(curPrice, 'd', 5), font);
    textRightPrice->setDefaultTextColor(Qt::white);

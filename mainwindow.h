...
struct Bar{ //Структура для хранения каждого 15минутного участка
        ...
        int volume;
        QDateTime time;
        Bar(QDateTime time,double open,double high,double low,double close,int vol):time(time),open(open),high(high),low(low),close(close),volume(vol){}
};
struct BuySellPos{
    ...
    QDateTime timeClose;
    bool TPpassed;
    BuySellPos(QDateTime timeOpen,double open,double stopLoss,double takeProfit, bool direction):timeOpen(timeOpen),open(open),stopLoss(stopLoss),takeProfit(takeProfit),direction(direction){}
...
class MainWindow : public QMainWindow
{
    ...
    bool resizeMark=true; //WheelEvent вызывается дважды. Когда тру - ресайз и меняем на фолс. Когда фолс - только замена на тру.
    bool graphEnd=false;  //Вводим булеву для определения, стоит ли график в конце, если да, то при ресайзе, двигается левая сторона, а не правая.
                          //Если нет, то бары добавляются и исчезают справа.
    double screenSize;
    double maxSet;
    double minSet;
    int pos = 3;
    int sceneWidth = 600;
    int sceneheight = 480;
    int paddingTop = 70;    //Отступ от края сцены сверху - от него рисуются элементы графика
    int paddingBottom=30;    //Отступ от края сцены снизу - до него рисуются элементы графика
...
//    bool crossArrow = false;
    ...
    QVector<int> barScrollList{4,8,16,32,64,128};     //Количество баров для скролла
    int barIndex=1;                              //Индекс какой размер бара и расстояние сейчас актуален (13 размер и 2 пикселя расстояние)

    bool ctrlPressed=false;
    double barResize = 0;   /*Поскольку прокрутка колеса мыши вызывает wheelevent дважды, то ввожу barResize,
                            который на каждую прокрутку будет увеличиваться на 0.5 и операция ресайза будет выполняться один раз за прокрутку колеса мыши. При значении 1, barSize обнуляется.*/
